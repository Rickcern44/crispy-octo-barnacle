<script lang="ts">
	...
	import { displayProgress, epicForItem, milestoneDate, milestoneKind, milestones, roadmap, shippedFeatureCount } from '$lib/roadmap';
...
<main class="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
	...
	<section class="mt-10" aria-labelledby="work-title">
		...
		<div class="mt-4 divide-y divide-slate-800 overflow-hidden rounded-xl border border-slate-800 bg-slate-900/60">{#if filteredItems.length > 0}{#each filteredItems as item}<a class="roadmap-row" href={`${base}/roadmap/rm-${item.id}/`}><div class="min-w-0"><p class="text-xs font-semibold tracking-wider text-cyan-300">RM-{item.id} · {item.feature_type}{#if epicForItem(item)} · {epicForItem(item)?.title}{/if}</p><h3 class="mt-1 font-semibold text-white">{item.title}</h3><p class="mt-1 line-clamp-2 text-sm text-slate-400">{item.description || item.current_state || item.rationale || 'No summary recorded.'}</p></div><div class="roadmap-row-meta"><div class="roadmap-progress"><div class="progress-track" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow={displayProgress(item)} aria-label={`${displayProgress(item)} percent complete`}><span style={`width: ${displayProgress(item)}%`}></span></div><span class="progress-value">{displayProgress(item)}%</span></div><span class="text-sm text-slate-400">{item.horizon}</span><span class="status-badge">{item.status}</span></div></a>{/each}{:else}<p class="p-5 text-sm text-slate-400">No items match this view.</p>{/if}</div>
	</section>
